...
const SalesReturnDialog = ({ invoice, open, onOpenChange }: SalesReturnDialogProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data, error } = await (supabase.rpc as any)("return_sales_invoice", {
      ...
      <DialogContent className={`max-w-3xl max-h-[90vh] overflow-y-auto ${isRTL ? "rtl" : "ltr"}`}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isRTL ? "مرتجع مبيعات" : "Sales Return"}
          </DialogTitle>
        </DialogHeader>
