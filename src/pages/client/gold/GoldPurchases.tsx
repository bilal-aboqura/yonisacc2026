...
import { Plus, Search, ShoppingCart, Weight, DollarSign, CheckCircle } from "lucide-react";
...
const GoldPurchases = () => {
  ...
  const { data: invoices = [], isLoading } = useQuery({
    queryKey: ["gold-purchases", companyId],
    queryFn: async () => {
      if (!companyId) return [];
      const { data, error } = await supabase
        .from("gold_purchase_invoices" as any)
  ...
  return (
    <div className="space-y-6" dir={isRTL ? "rtl" : "ltr"}>
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold">{isRTL ? "فواتير شراء الذهب" : "Gold Purchase Invoices"}</h1>
          <p className="text-muted-foreground">{isRTL ? "إدارة مشتريات الذهب" : "Manage gold purchases"}</p>
        </div>
        <Button onClick={() => navigate("/client/gold/purchases/new")} className="bg-amber-600 hover:bg-amber-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card><CardContent className="p-4 flex items-center gap-3">
          <div className="p-2 rounded-lg bg-blue-100 dark:bg-blue-900/30"><ShoppingCart className="h-5 w-5 text-blue-600" /></div>
