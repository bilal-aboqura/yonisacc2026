...
const CreateGoldSale = () => {
  ...
  const { data: customers = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!companyId) return [];
      const { data } = await supabase.from("contacts").select("id, name, name_en").eq("company_id", companyId).eq("type", "customer").eq("is_active", true);
      return data || [];
    },
    enabled: !!companyId,
  });
  ...
  return (
    <div className="space-y-6" dir={isRTL ? "rtl" : "ltr"}>
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate("/client/gold/sales")}>
          <ArrowLeft className="h-5 w-5" />
        </Button>
        <h1 className="text-2xl font-bold">{isRTL ? "فاتورة بيع ذهب جديدة" : "New Gold Sales Invoice"}</h1>
      </div>

      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Receipt className="h-5 w-5 text-amber-500" />{isRTL ? "بيانات الفاتورة" : "Invoice Details"}</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium">{isRTL ? "العميل" : "Customer"}</label>
      ...
      <div className="flex gap-3">
        ...
        <Button variant="outline" onClick={() => navigate("/client/gold/sales")}>{isRTL ? "إلغاء" : "Cancel"}</Button>
      </div>
    </div>
  );
};

export default CreateGoldSale;
