...
import { ArrowRight, ArrowLeft, Printer, Loader2, CheckCircle2, AlertTriangle, ChevronDown, ChevronLeft, ChevronRight } from "lucide-react";
...
const BalanceSheet = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [accountsRes, balanceRes] = await Promise.all([
        supabase
          .from("accounts")
          .select("id, code, name, name_en, type, parent_id, is_parent, is_active")
      ...
      // Auto-expand root parent nodes for asset/liability/equity
      const rootIds = new Set<string>();
      (accountsRes.data || []).forEach((a: RawAccount) => {
        if (!a.parent_id && a.is_parent && ["asset", "liability", "equity"].includes(a.type)) {
          rootIds.add(a.id);
        }
      });
  ...
  // Compute net income from revenue/expense for equity linkage
  ...
  const totalRevenue = revenueAccounts
    ...
    .reduce((s, a) => s + Math.abs(balanceMap.get(a.id) || 0), 0);
  const totalExpenseVal = expenseAccounts
    .filter((a) => !a.is_parent)
    .reduce((s, a) => s + Math.abs(balanceMap.get(a.id) || 0), 0);
  ...
  return (
    <div className={`space-y-6 ${isRTL ? "rtl" : "ltr"}`}>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-blue-600">{isRTL ? "الأصول" : "Assets"}</CardTitle>
          </CardHeader>
          <CardContent>
            {assetTree.length === 0 ? (
              <p className="text-center text-muted-foreground py-8">{isRTL ? "لا توجد أصول" : "No assets"}</p>
            ) : (
              <div className="space-y-0.5">{renderNodes(assetTree, "text-blue-600")}</div>
            )}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-red-600">{isRTL ? "الخصوم" : "Liabilities"}</CardTitle>
            </CardHeader>
            <CardContent>
              {liabilityTree.length === 0 ? (
                <p className="text-center text-muted-foreground py-8">{isRTL ? "لا توجد خصوم" : "No liabilities"}</p>
              ) : (
                <div className="space-y-0.5">{renderNodes(liabilityTree, "text-red-600")}</div>
              )}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-green-600">{isRTL ? "حقوق الملكية" : "Equity"}</CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  <div className="flex items-center justify-between py-2 px-3 bg-muted/30 rounded-md mt-2 italic">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        {netIncome >= 0
                          ? isRTL ? "صافي ربح الفترة (قائمة الدخل)" : "Net Profit (Income Statement)"
                          : isRTL ? "صافي خسارة الفترة (قائمة الدخل)" : "Net Loss (Income Statement)"}
                      </span>
                    </div>
                    <span className={`font-mono text-sm ${netIncome >= 0 ? "text-green-600" : "text-destructive"}`}>
